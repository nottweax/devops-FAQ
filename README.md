...
Nested Loop - цикл в цикле(сравнение в двух таблицах)

Hash Join - каждое значаение из 1 таблицы попадает черех хеш функцию в дополнительную таблицу, потом каждое значение из 2 таблицы проходит через ту же хеш функцию и складывается в дополнительную таблицу

Sort Merge Join (с повторение / без повторений) - **обязательно сортированные таблицы** значение первой таблицы по очереди сравнивается со значением из второй таблицы
